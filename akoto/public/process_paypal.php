<?php
        ...
        {
                if(!isset($_SESSION['your_form']) || !isset($_SESSION["tax"]) || !isset($_SESSION["shipping_cost"]) )
                {
                        error_log("unable to retrieve session-your_form,tax,shippingcost. user redirected to checkout.php");
                ...
                {
                        error_log("session expired. unable to retrieve session-tax");
                        $message = "Your session has expired.Please re-enter your order. Thank you";
                ...
                {
                        error_log("session expired. unable to retrieve session-shipping_cost");
                        $message = "Your session has expired.Please re-enter your order. Thank you";
                        $params = array("title" => "error", "message" => $message);
                        render("apology.php",  $params );
                        exit(); 
                ...
                if(!isset($_POST['item_name']) || !isset($_POST['item_code']) || !isset($_POST['item_qty']) )
                {
                        error_log("post items is not set in process_paypal.php.");
                        $message = "There was a problem. Please re-enter your order. Thank you";
                        $params = array("title" => "error", "message" => $message);
                        render("apology.php",  $params );
                        exit(); 
                ...
                foreach($_POST['item_name'] as $key=>$itmname)
                {
                        ...
                        if (!$results )
                        {
                                error_log("database fetch is empty : error getting data from products db: process_paypal");
                        ...
                        if($results[0]["quantity"] <= 0)
                        {
                                ...
                                {
                                        error_log("session expired. unable to retrieve session-products");
                                        $message = "Your session has expired.Please re-enter your order. Thank you";
                                        $params = array("title" => "error", "message" => $message);
                                        render("apology.php",  $params );
                                        exit(); 
		...
		$mysqli->setAttribute(PDO::ATTR_EMULATE_PREPARES, false); 
			...
			error_log(print_r($mysqli->errorInfo()));
                        $message = "Problem processing order. Pls try again.";
                        $params = array("title" => "error", "message" => $message);
                        render("apology.php",  $params );
                        exit(); 
		...
		if(!$stmt->execute(array(':id' => $transaction_buyerid,
					 ...
					 ':status' => $status)))
		{
                        error_log( "unable to insert into pendingtransactions db. process_paypal");
                        error_log(print_r($mysqli->errorInfo()));
                        $message = "Problem processing order. Pls try again.";
                        $params = array("title" => "error", "message" => $message);
                        render("apology.php",  $params );
